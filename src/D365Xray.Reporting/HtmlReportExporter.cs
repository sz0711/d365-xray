...
internal sealed class HtmlReportExporter
{
    ...
    internal static string Build(RiskReport report)
    {
        ...
        sb.AppendLine("</div>");
        sb.AppendLine("<button id=\"print-btn\" class=\"theme-btn\" title=\"Print / PDF\" aria-label=\"Print report\" onclick=\"window.print()\">&#128438;</button>");
        sb.AppendLine("<button id=\"theme-toggle\" class=\"theme-btn\" title=\"Toggle dark mode\" aria-label=\"Toggle dark mode\">&#9790;</button>");
        sb.AppendLine("</div>");
        sb.AppendLine("</header>");

        // TOC sidebar
        AppendTableOfContents(sb, report);
        ...
        // ALM Maturity Score
        AppendAlmMaturityScore(sb, report);
    ...
    private static void AppendDashboard(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"dashboard\" class=\"dashboard\">");
        ...
        AppendKpiCard(sb, "Findings", report.Findings.Count.ToString(CultureInfo.InvariantCulture), "total", "findings");
        ...
        AppendKpiCard(sb, "Critical", critical.ToString(CultureInfo.InvariantCulture), "critical", "findings");
        AppendKpiCard(sb, "High", high.ToString(CultureInfo.InvariantCulture), "high", "findings");
        AppendKpiCard(sb, "Environments", report.ComparedEnvironments.Count.ToString(CultureInfo.InvariantCulture), "total", "env-inventory");
        ...
        AppendKpiCard(sb, "Custom Solutions", customSolCount.ToString(CultureInfo.InvariantCulture), "total", "solution-breakdown");
        AppendKpiCard(sb, "Unmanaged", unmanagedCount.ToString(CultureInfo.InvariantCulture), unmanagedCount > 0 ? "high" : "total", "solution-breakdown");
        sb.AppendLine("</div>");

        sb.AppendLine("</section>");
    }

    private static void AppendKpiCard(StringBuilder sb, string label, string value, string cssClass, string? targetId = null)
    {
        var dataAttr = targetId is not null ? $" data-target=\"{targetId}\"" : "";
        sb.AppendLine(CultureInfo.InvariantCulture,
            $"<div class=\"kpi-card {cssClass}\"{dataAttr}><span class=\"kpi-value\">{value}</span><span class=\"kpi-label\">{Encode(label)}</span></div>");
    }
    ...
    {
        sb.AppendLine("<section id=\"env-inventory\" class=\"env-inventory\">");
    ...
    {
        sb.AppendLine("<section id=\"env-inventory\" class=\"environments\">");
    ...
    private static void AppendSolutionBreakdown(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"solution-breakdown\" class=\"solution-breakdown\">");
    ...
    private static void AppendCustomArtifacts(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"custom-artifacts\" class=\"custom-artifacts\">");
    ...
    {
        sb.AppendLine("<section id=\"executive-summary\" class=\"executive-summary\">");
    ...
    private static void AppendSettingsAudit(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"settings-audit\" class=\"settings-audit\">");
    ...
    private static void AppendPluginRegistrationMap(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"plugin-map\" class=\"plugin-map\">");
    ...
    private static void AppendSecurityPosture(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"security-posture\" class=\"security-posture\">");
    ...
    private static void AppendEnvironmentVariables(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"env-variables\" class=\"env-variables\">");
    ...
    private static void AppendEntityGovernance(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"entity-governance\" class=\"entity-governance\">");
    ...
    private static void AppendSeveritySection(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"severity-summary\" class=\"severity-summary\">");
    ...
    private static void AppendAnalysisCoverage(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"analysis-coverage\" class=\"analysis-coverage\">");
    ...
    private static void AppendFindings(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"findings\" class=\"findings\">");
    ...
    private static void AppendAiSection(StringBuilder sb, RiskReport report)
    {
        ...
        sb.AppendLine("<section id=\"ai-section\" class=\"ai-section\">");
    ...
    private static void AppendTableOfContents(StringBuilder sb, RiskReport report)
    {
        sb.AppendLine("<nav id=\"toc\" class=\"toc\">");
        sb.AppendLine("<div class=\"toc-header\"><strong>Contents</strong><button class=\"toc-close\" onclick=\"document.getElementById('toc').classList.remove('open')\" aria-label=\"Close\">&times;</button></div>");
        sb.AppendLine("<ul>");
        sb.AppendLine("<li><a href=\"#dashboard\">Dashboard</a></li>");
        sb.AppendLine("<li><a href=\"#alm-maturity\">ALM Maturity</a></li>");
        sb.AppendLine("<li><a href=\"#executive-summary\">Executive Summary</a></li>");
        sb.AppendLine("<li><a href=\"#env-inventory\">Environments</a></li>");
        sb.AppendLine("<li><a href=\"#solution-breakdown\">Solutions</a></li>");
        sb.AppendLine("<li><a href=\"#custom-artifacts\">Custom Artifacts</a></li>");

        if (report.SettingsSnapshots.Count > 0)
        {
            sb.AppendLine("<li><a href=\"#settings-audit\">Settings Audit</a></li>");
        }

        if (report.PluginMaps.Count > 0)
        {
            sb.AppendLine("<li><a href=\"#plugin-map\">Plugin Map</a></li>");
        }

        if (report.SecurityPostures.Count > 0)
        {
            sb.AppendLine("<li><a href=\"#security-posture\">Security Posture</a></li>");
        }

        if (report.EnvironmentVariableInventories.Count > 0)
        {
            sb.AppendLine("<li><a href=\"#env-variables\">Env Variables</a></li>");
        }

        if (report.EntityGovernances.Count > 0)
        {
            sb.AppendLine("<li><a href=\"#entity-governance\">Entity Governance</a></li>");
        }

        sb.AppendLine("<li><a href=\"#severity-summary\">Severity</a></li>");
        sb.AppendLine("<li><a href=\"#analysis-coverage\">Coverage</a></li>");
        sb.AppendLine("<li><a href=\"#findings\">Findings</a></li>");

        if (report.AiEnrichment is not null)
        {
            sb.AppendLine("<li><a href=\"#ai-section\">AI Analysis</a></li>");
        }

        sb.AppendLine("<li><a href=\"#action-items\">Action Items</a></li>");
        sb.AppendLine("</ul>");
        sb.AppendLine("</nav>");
        sb.AppendLine("<button id=\"toc-toggle\" class=\"toc-toggle-btn\" aria-label=\"Toggle table of contents\" onclick=\"document.getElementById('toc').classList.toggle('open')\">&#9776;</button>");
    }

    // ── ALM Maturity Score ──────────────────────────────────

    private static void AppendAlmMaturityScore(StringBuilder sb, RiskReport report)
    {
        // Calculate dimension scores (0-100 each)
        var dimensions = new List<(string Name, int Score, string Icon, string Detail)>();

        // 1. Solution Management: % of custom solutions that are managed
        var allCustomSols = report.SolutionInventories.SelectMany(i => i.Solutions).Where(s => !s.IsMicrosoft).ToList();
        if (allCustomSols.Count > 0)
        {
            var managedPct = allCustomSols.Count(s => s.IsManaged) * 100 / allCustomSols.Count;
            dimensions.Add(("Solution Management", managedPct, "&#128230;", $"{allCustomSols.Count(s => s.IsManaged)}/{allCustomSols.Count} managed"));
        }

        // 2. Plugin Architecture: % async (sync = risk, async = good)
        foreach (var map in report.PluginMaps)
        {
            if (map.TotalSteps > 0)
            {
                var activeSteps = map.TotalSteps - map.DisabledSteps;
                var asyncPct = activeSteps > 0 ? map.AsyncSteps * 100 / activeSteps : 100;
                dimensions.Add(("Plugin Architecture", asyncPct, "&#9889;", $"{map.AsyncSteps}/{activeSteps} async"));
            }
        }

        // 3. Entity Governance: audit coverage %
        foreach (var gov in report.EntityGovernances)
        {
            if (gov.TotalCustomEntities > 0)
            {
                var auditPct = gov.AuditEnabledCount * 100 / gov.TotalCustomEntities;
                dimensions.Add(("Audit Coverage", auditPct, "&#128269;", $"{gov.AuditEnabledCount}/{gov.TotalCustomEntities} audited"));
            }
        }

        // 4. Environment Variables: % with values
        foreach (var inv in report.EnvironmentVariableInventories)
        {
            if (inv.Variables.Count > 0)
            {
                var withValue = inv.Variables.Count(v => v.HasValue);
                var valPct = withValue * 100 / inv.Variables.Count;
                dimensions.Add(("Env Configuration", valPct, "&#9881;", $"{withValue}/{inv.Variables.Count} configured"));
            }
        }

        // 5. Settings Compliance: % of assessed settings that are OK
        foreach (var ss in report.SettingsSnapshots)
        {
            if (ss.Settings.Count > 0)
            {
                var okCount = ss.Settings.Count(s => AssessSettingValue(s).Assessment.Contains("low", StringComparison.OrdinalIgnoreCase));
                var compliancePct = okCount * 100 / ss.Settings.Count;
                dimensions.Add(("Settings Compliance", compliancePct, "&#128736;", $"{okCount}/{ss.Settings.Count} compliant"));
            }
        }

        // 6. Security Posture: % of roles that are managed
        foreach (var posture in report.SecurityPostures)
        {
            if (posture.TotalRoles > 0)
            {
                var managedPct = (posture.TotalRoles - posture.UnmanagedRoleCount) * 100 / posture.TotalRoles;
                dimensions.Add(("Security Governance", managedPct, "&#128274;", $"{posture.TotalRoles - posture.UnmanagedRoleCount}/{posture.TotalRoles} managed"));
            }
        }
        ...
        var overallScore = (int)dimensions.Average(d => d.Score);
        ...
        var levelColor = overallScore switch
        {
            >= 80 => "low",
            >= 60 => "medium",
            >= 40 => "medium",
        ...
        sb.AppendLine("<section id=\"alm-maturity\" class=\"alm-maturity\">");
        sb.AppendLine("<h2>ALM Maturity Score</h2>");
        sb.AppendLine("<div class=\"maturity-overview\">");

        // Overall score badge
        sb.AppendLine(CultureInfo.InvariantCulture,
            $"<div class=\"maturity-badge\"><span class=\"maturity-score\" style=\"color:var(--color-{levelColor})\">{overallScore}</span><span class=\"maturity-level\">{maturityLevel}</span><span class=\"maturity-subtitle\">{dimensions.Count} dimensions</span></div>");

        // Dimension bars
        sb.AppendLine("<div class=\"maturity-dimensions\">");
        foreach (var (name, score, icon, detail) in dimensions)
        {
            ...
            sb.AppendLine(CultureInfo.InvariantCulture,
                $"<div class=\"maturity-dim\"><div class=\"maturity-dim-header\"><span>{icon} {Encode(name)}</span><span class=\"maturity-dim-detail\">{Encode(detail)}</span></div><div class=\"maturity-bar-track\"><div class=\"maturity-bar-fill\" style=\"width:{score}%;background:{barColor}\"></div></div><span class=\"maturity-pct\">{score}%</span></div>");
        }
    ...
    private static void AppendActionItems(StringBuilder sb, RiskReport report)
    {
        var items = new List<(string Priority, string PriorityClass, string Title, string Description, string Section)>();

        // Critical/High findings → action items
        var criticalCount = report.SeverityCounts.GetValueOrDefault(Severity.Critical);
        var highCount = report.SeverityCounts.GetValueOrDefault(Severity.High);
        if (criticalCount > 0)
        {
            items.Add(("P1", "critical", "Resolve Critical Findings",
                $"{criticalCount} critical finding(s) require immediate attention. Review the Findings section for details.", "findings"));
        }

        if (highCount > 0)
        {
            items.Add(("P2", "high", "Address High-Severity Findings",
                $"{highCount} high-severity finding(s) should be planned for the next sprint.", "findings"));
        }

        // Settings-based action items
        foreach (var ss in report.SettingsSnapshots)
        {
            var auditSetting = ss.Settings.FirstOrDefault(s => s.Key == "isauditenabled");
            if (auditSetting is { Value: "False" or "false" })
            {
                items.Add(("P1", "critical", "Enable Organization Auditing",
                    "Auditing is disabled. Enable it in Settings &gt; Audit Settings for compliance and traceability.", "settings-audit"));
            }

            var sessionTimeout = ss.Settings.FirstOrDefault(s => s.Key == "sessiontimeoutenabled");
            if (sessionTimeout is { Value: "False" or "false" })
            {
                items.Add(("P2", "high", "Enable Session Timeout",
                    "Session timeout is not enforced. Enable in Settings &gt; Administration &gt; System Settings.", "settings-audit"));
            }

            var pluginTrace = ss.Settings.FirstOrDefault(s => s.Key == "plugintracelogsetting");
            if (pluginTrace is { Value: "2" })
            {
                items.Add(("P2", "high", "Reduce Plugin Trace Logging",
                    "Plugin trace logging is set to All, causing performance overhead. Change to Exception in production.", "settings-audit"));
            }

            var blocked = ss.Settings.FirstOrDefault(s => s.Key == "blockedattachments");
            if (blocked is not null && string.IsNullOrWhiteSpace(blocked.Value))
            {
                items.Add(("P2", "high", "Configure Blocked Attachments",
                    "No attachment blocklist is configured. Add file extensions to prevent malicious uploads.", "settings-audit"));
            }
        }
        ...
        foreach (var map in report.PluginMaps)
        {
            if (map.SyncSteps > 0)
            {
                items.Add(("P3", "medium", "Review Synchronous Plugins",
                    $"{map.SyncSteps} synchronous plugin step(s) detected. Evaluate converting to async for better UX.", "plugin-map"));
            }

            if (map.DisabledSteps > 0)
            {
                items.Add(("P3", "medium", "Clean Up Disabled Plugins",
                    $"{map.DisabledSteps} disabled plugin step(s). Remove if no longer needed to reduce complexity.", "plugin-map"));
            }
        }

        // Unmanaged security roles
        foreach (var posture in report.SecurityPostures)
        {
            if (posture.UnmanagedRoleCount > 0)
            {
                items.Add(("P2", "high", "Manage Security Roles in Solutions",
                    $"{posture.UnmanagedRoleCount} unmanaged security role(s). Include in managed solutions for ALM governance.", "security-posture"));
            }
        }

        // Missing env variable values
        foreach (var inv in report.EnvironmentVariableInventories)
        {
            if (inv.MissingValueCount > 0)
            {
                items.Add(("P2", "high", "Set Environment Variable Values",
                    $"{inv.MissingValueCount} environment variable(s) missing values. Configure to avoid runtime failures.", "env-variables"));
            }
        }
        ...
        foreach (var gov in report.EntityGovernances)
        {
            if (gov.TotalCustomEntities > 0)
            {
                ...
                if (auditPct < 50)
                {
                    items.Add(("P2", "high", "Increase Entity Audit Coverage",
                        $"Only {auditPct}% of custom entities have auditing enabled. Target &ge; 80% for compliance.", "entity-governance"));
                }
            }
        }

        // Default solution warning
        var defaultSol = report.SolutionInventories
            .SelectMany(i => i.Solutions)
            .FirstOrDefault(s => s.UniqueName.Equals("Default", StringComparison.OrdinalIgnoreCase));
        if (defaultSol is not null && defaultSol.TotalComponents > 1000)
        {
            items.Add(("P2", "high", "Consolidate Default Solution",
                $"Default Solution has {defaultSol.TotalComponents.ToString("N0", CultureInfo.InvariantCulture)} components. Migrate customizations to dedicated managed solutions.", "solution-breakdown"));
        }
        ...
        sb.AppendLine("<section id=\"action-items\" class=\"action-items\">");
        sb.AppendLine("<h2>Action Items &amp; Remediation Roadmap</h2>");
        sb.AppendLine("<p class=\"meta\">Prioritized recommendations derived from scan findings. Items are ranked P1 (immediate) through P3 (planned improvement).</p>");
        sb.AppendLine("<div class=\"action-list\">");

        foreach (var (priority, priorityClass, title, description, section) in items)
        {
            sb.AppendLine(CultureInfo.InvariantCulture,
                $"<div class=\"action-item\"><span class=\"tag {priorityClass}\">{priority}</span> <strong>{Encode(title)}</strong><p>{description} <a href=\"#{section}\" class=\"deep-link\">View section &rarr;</a></p></div>");
        }
    ...
    private static void AppendScripts(StringBuilder sb, RiskReport report)
    {
        ...
        // KPI card click-to-jump
        sb.AppendLine("""
        (function(){
          document.querySelectorAll('.kpi-card[data-target]').forEach(function(card){
        ...
        (function(){
          ...
          var observer=new IntersectionObserver(function(entries){
            entries.forEach(function(entry){
              var link=sections.find(function(s){return s.el===entry.target;});
              if(link){
        ...
        // Auto-show TOC on wide screens
        sb.AppendLine("""
        (function(){
          var toc=document.getElementById('toc');
          if(!toc)return;
          function checkWidth(){
            if(window.innerWidth>=1560)toc.classList.add('open');
          }
          checkWidth();
        ...
        sb.AppendLine("""
            ...
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .toc {
              ...
              overflow-y: auto; padding: 1rem .75rem; z-index: 200;
              transform: translateX(-100%); transition: transform .25s ease;
              box-shadow: 2px 0 12px rgba(0,0,0,.1); font-size: .82rem;
            }
            .toc.open { transform: translateX(0); }
            .toc-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: .75rem; padding-bottom: .5rem; border-bottom: 1px solid var(--border); }
            .toc-close { background: none; border: none; color: var(--muted); font-size: 1.3rem; cursor: pointer; line-height: 1; }
            .toc ul { list-style: none; padding: 0; }
            .toc li { margin: 0; }
            .toc a { display: block; padding: .35rem .5rem; color: var(--text); text-decoration: none; border-radius: .25rem; border-left: 2px solid transparent; transition: background .15s, border-color .15s; }
            .toc a:hover { background: var(--gauge-bg); }
            .toc a.active { border-left-color: #5b8def; color: #5b8def; font-weight: 600; background: rgba(91,141,239,.06); }
            .toc-toggle-btn {
              position: fixed; bottom: 1.5rem; left: 1.5rem; width: 44px; height: 44px;
              border-radius: 50%; background: #5b8def; color: #fff; border: none;
              font-size: 1.2rem; cursor: pointer; z-index: 201;
              box-shadow: 0 2px 8px rgba(0,0,0,.2); transition: background .2s;
            }
            .toc-toggle-btn:hover { background: #4a7de0; }

            /* ALM Maturity */
            .maturity-overview { display: flex; gap: 2rem; align-items: flex-start; flex-wrap: wrap; margin: .75rem 0; }
            .maturity-badge { text-align: center; min-width: 120px; padding: 1rem; background: var(--card); border: 1px solid var(--border); border-radius: .75rem; }
            .maturity-score { display: block; font-size: 3rem; font-weight: 800; line-height: 1; }
            .maturity-level { display: block; font-size: 1rem; font-weight: 600; color: var(--muted); text-transform: uppercase; letter-spacing: .05em; margin-top: .25rem; }
            .maturity-subtitle { display: block; font-size: .75rem; color: var(--muted); margin-top: .2rem; }
            .maturity-dimensions { flex: 1; min-width: 280px; }
            .maturity-dim { margin: .5rem 0; }
            .maturity-dim-header { display: flex; justify-content: space-between; font-size: .85rem; margin-bottom: .2rem; }
            .maturity-dim-detail { color: var(--muted); font-size: .8rem; }
            .maturity-bar-track { height: 8px; background: var(--gauge-bg); border-radius: 4px; overflow: hidden; flex: 1; }
            .maturity-bar-fill { height: 100%; border-radius: 4px; transition: width .6s ease; }
            .maturity-pct { font-size: .8rem; font-weight: 600; min-width: 35px; text-align: right; }
            .maturity-dim { display: grid; grid-template-columns: 1fr auto; gap: .3rem .5rem; align-items: center; }
            .maturity-dim-header { grid-column: 1 / -1; }

            /* Action Items */
            .action-list { display: flex; flex-direction: column; gap: .5rem; }
            .action-item { padding: .75rem 1rem; background: var(--card); border: 1px solid var(--border); border-radius: .5rem; }
            .action-item p { font-size: .9rem; color: var(--text); margin: .3rem 0 0; }
            .action-item .deep-link { margin-left: .5rem; }

            /* Print */
            @media print {
              body { max-width: none; padding: 0; background: #fff; color: #000; }
              body.dark { --bg: #fff; --card: #fff; --border: #ccc; --text: #000; --muted: #555; }
              .toc, .toc-toggle-btn, #theme-toggle, #print-btn, .filter-bar, .theme-btn { display: none !important; }
              header { background: #1a1a2e !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; break-after: avoid; }
